...
class SIMPLIFIED_fromZero_feedforward_multiscale_tempo_Matt_sepConv_SpikeFlowNetLike(NeuromorphicNet):
    ...
    Modified to provide ofmaps (after activation function)
    ...
    def forward(self, x):
        ...
        return (
                ...
                [out_bottom, out_conv1, out_conv2, out_conv3, out_conv4]
               )

class Simplified_StereoSpike_bin(NeuromorphicNet):
    """
    Simplified but mathematically equivalent version of 'fromZero_feedforward_multiscale_tempo_Matt_sepConv_SpikeFlowNetLike'
        - It enforces that all event frames have only binary values
        - Modified to provide ofmaps (after activation function)
    ...
    def forward(self, x):
        ...
        frame = torch.clamp(x[:, 0, :, :, :], min=0, max=1)  
