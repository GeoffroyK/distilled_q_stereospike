"""
Modified to get intermediate results in the quantized model
"""
...
class QUANTIZABLE_SIMPLIFIED_fromZero_feedforward_multiscale_tempo_Matt_sepConv_SpikeFlowNetLike(NeuromorphicNet):
    ...
    Modification (Andres) --> extract intermediate results before and after activation function.
    ...
    def forward(self, x):
        ...
        out_bottom = self.quant(torch.heaviside(self.dequant(out_bottom_int) - 1., torch.tensor(0.0, device=x.device)))
        ...
        """
        out_conv1_int = self.conv1(out_bottom)
        out_conv1 = self.quant(torch.heaviside(self.dequant(out_conv1_int) - 1., torch.tensor(0.0, device=x.device)))
        out_conv2_int = self.conv2(out_conv1)
        out_conv2 = self.quant(torch.heaviside(self.dequant(out_conv2_int) - 1., torch.tensor(0.0, device=x.device)))
        out_conv3_int = self.conv3(out_conv2)
        out_conv3 = self.quant(torch.heaviside(self.dequant(out_conv3_int) - 1., torch.tensor(0.0, device=x.device)))
        out_conv4_int = self.conv4(out_conv3)
        out_conv4 = self.quant(torch.heaviside(self.dequant(out_conv4_int) - 1., torch.tensor(0.0, device=x.device)))
        ...
        """
        We include two more lists: binary tensors (after activation function) and integer tensors (before activation functions)
        at this moment, it only considers stem and encoder
        """
        return (
                [depth1, depth2, depth3, depth4], 
                [out_rconv, out_add4, out_add3, out_add2, out_add1], 
                [frame, out_bottom, out_conv1, out_conv2, out_conv3, out_conv4],
                [out_bottom_int, out_conv1_int, out_conv2_int, out_conv3_int, out_conv4_int]
                )
