'''
    Script provided by Tomo with the StereoSpike Model without skip connections (pretrained model) 
'''
...
from .blocks import SEWResBlock, NNConvUpsampling, MultiplyBy, SeparableSEWResBlock, SeparableNNConvUpsampling#, SeparableSEWResBlock_Group1
...
#####################
# This is no skipconnection network
#####################
class fromZero_feedforward_multiscale_tempo_Matt_noskip_SpikeFlowNetLike(NeuromorphicNet):
    ...
    IT IS FULLY SPIKING AND HAS THE BEST MDE WE'VE SEEN SO FAR !!!
        ...
        - There are skip connections in the bottleneck --> check more carefully this part again!!!!
...
class fromZero_feedforward_multiscale_tempo_Matt_sepConv_SpikeFlowNetLike(NeuromorphicNet):
    ...
    def forward(self, x):
        ...
        leaf = list()
        frame = x[:, 0, :, :, :]
        leaf.append(frame.is_leaf)
        
        

        # data is fed in through the bottom layer
        out_bottom = self.bottom(frame)
        leaf.append(out_bottom.is_leaf)

        # pass through encoder layers
        out_conv1 = self.conv1(out_bottom)
        leaf.append(out_conv1.is_leaf)
        out_conv2 = self.conv2(out_conv1)
        leaf.append(out_conv2.is_leaf)
        out_conv3 = self.conv3(out_conv2)
        leaf.append(out_conv3.is_leaf)
        out_conv4 = self.conv4(out_conv3)
        leaf.append(out_conv4.is_leaf)

        # pass through residual blocks
        out_rconv = self.bottleneck(out_conv4)
        leaf.append(out_rconv.is_leaf)

        # gradually upsample while concatenating and passing through skip connections
        out_deconv4 = self.deconv4(out_rconv)
        leaf.append(out_deconv4.is_leaf)
        print("leaf: ", leaf)
        out_add4 = out_deconv4 + out_conv3
        self.Ineurons(self.predict_depth4(out_add4))
        depth4 = self.Ineurons.v
...
class quantized_OPT_fromZero_feedforward_multiscale_tempo_Matt_sepConv_SpikeFlowNetLike(NeuromorphicNet):
    ...
    def forward(self, x):
        ...
        # pass through residual blocks
        out_rconv = self.bottleneck(out_conv4)  #out_conv4  # self.bottleneck(out_conv4)
...
class Quant_fromZero_feedforward_multiscale_tempo_Matt_sepConv_SpikeFlowNetLike(NeuromorphicNet):
    """
    Uses separable convolutions for a lighter model
    ...
    def forward(self, x):
        ...
        # data is fed in through the bottom layer
        out_bottom = self.bottom(frame).detach()
        leaf.append(out_bottom.is_leaf)

        # pass through encoder layers
        out_conv1 = self.conv1(out_bottom).detach()
        leaf.append(out_conv1.is_leaf)
        out_conv2 = self.conv2(out_conv1).detach()
        leaf.append(out_conv2.is_leaf)
        out_conv3 = self.conv3(out_conv2).detach()
        leaf.append(out_conv3.is_leaf)
        out_conv4 = self.conv4(out_conv3).detach()
        leaf.append(out_conv4.is_leaf)

        # pass through residual blocks
        out_rconv = self.bottleneck(out_conv4).detach()
        leaf.append(out_rconv.is_leaf)

        # gradually upsample while concatenating and passing through skip connections
        out_deconv4 = self.deconv4(out_rconv).detach()
        leaf.append(out_deconv4.is_leaf)
        out_add4 = out_deconv4 + out_conv3
        leaf.append(out_add4.is_leaf)
        self.Ineurons(self.predict_depth4(out_add4))
        depth4 = self.Ineurons.v.detach()
        leaf.append(depth4.is_leaf)
        

        out_deconv3 = self.deconv3(out_add4).detach()
        leaf.append(out_deconv3.is_leaf)
        out_add3 = out_deconv3 + out_conv2
        leaf.append(out_add3.is_leaf)
        self.Ineurons(self.predict_depth3(out_add3))
        depth3 = self.Ineurons.v.detach()
        leaf.append(depth3.is_leaf)

        out_deconv2 = self.deconv2(out_add3).detach()
        leaf.append(out_deconv2.is_leaf)
        out_add2 = out_deconv2 + out_conv1
        leaf.append(out_add2.is_leaf)
        self.Ineurons(self.predict_depth2(out_add2))
        depth2 = self.Ineurons.v.detach()
        leaf.append(depth2.is_leaf)

        out_deconv1 = self.deconv1(out_add2).detach()
        leaf.append(out_deconv1.is_leaf)
        out_add1 = out_deconv1 + out_bottom
        leaf.append(out_add1.is_leaf)
        self.Ineurons(self.predict_depth1(out_add1))
        depth1 = self.Ineurons.v.detach()
        leaf.append(depth1.is_leaf)
        print("leaf: ", leaf)
...
class PostTrainingQuantizedModel(fromZero_feedforward_multiscale_tempo_Matt_sepConv_SpikeFlowNetLike):
    """
    Post-Training Quantization of fromZero_feedforward_multiscale_tempo_Matt_sepConv_SpikeFlowNetLike.
    """
    ...
    def fuse_model(self):
        """
        Fuse Conv, BatchNorm, and ReLU for quantization.
        """
        for module in [self.bottom, self.conv1, self.conv2, self.conv3, self.conv4, 
                       self.deconv4, self.deconv3, self.deconv2, self.deconv1]:
            torch.quantization.fuse_modules(module, [['0', '1']], inplace=True)

    def forward(self, x):
        x = self.quant(x)
        print("quant")
        depth_outputs, activations = super().forward(x)
        depth1 = self.dequant(depth_outputs[0])
        depth2 = self.dequant(depth_outputs[1])
        depth3 = self.dequant(depth_outputs[2])
        depth4 = self.dequant(depth_outputs[3])
        out_rconv = self.dequant(activations[0])
        out_add4 = self.dequant(activations[1])
        out_add3 = self.dequant(activations[2])
        out_add2 = self.dequant(activations[3])
        out_add1 = self.dequant(activations[4])
        print("dequant")
        return [depth1, depth2, depth3, depth4], [out_rconv, out_add4, out_add3, out_add2, out_add1]


class Quant_PostTrainingQuantizedModel(Quant_fromZero_feedforward_multiscale_tempo_Matt_sepConv_SpikeFlowNetLike):
    """
    Post-Training Quantization of fromZero_feedforward_multiscale_tempo_Matt_sepConv_SpikeFlowNetLike.
    """
    def __init__(self, *args, **kwargs):
...
# class fromZero_feedforward_multiscale_tempo_Matt_sepConv_SpikeFlowNetLike_Group1(NeuromorphicNet):
#     """
#     Uses separable convolutions for a lighter model
...
#         self.bottom = nn.Sequential(
#             # nn.Conv2d(in_channels=4, out_channels=32, kernel_size=5, stride=1, padding=2, bias=False),
#             nn.Conv2d(in_channels=input_chans, out_channels=input_chans, groups=1, kernel_size=K, stride=1, padding=P, bias=False),
#             nn.Conv2d(in_channels=input_chans, out_channels=C[0], kernel_size=1, stride=1, bias=False),
#             MultiplyBy(multiply_factor),
#             neuron.ParametricLIFNode(init_tau=tau, v_threshold=v_threshold, v_reset=v_reset, detach_reset=True, surrogate_function=surrogate_function) if use_plif else neuron.LIFNode(tau=tau, v_threshold=v_threshold, v_reset=v_reset, surrogate_function=surrogate.ATan(), detach_reset=True),
#         )
...
#         self.conv1 = nn.Sequential(
#             # nn.Conv2d(in_channels=32, out_channels=64, kernel_size=5, stride=2, padding=2, bias=False),
#             nn.Conv2d(in_channels=C[0], out_channels=C[0], groups=1, kernel_size=K, stride=2, padding=P, bias=False),
#             nn.Conv2d(in_channels=C[0], out_channels=C[1], kernel_size=1, stride=1, bias=False),
#             MultiplyBy(multiply_factor),
#             neuron.ParametricLIFNode(init_tau=tau, v_threshold=v_threshold, v_reset=v_reset, detach_reset=True, surrogate_function=surrogate_function) if use_plif else neuron.LIFNode(tau=tau, v_threshold=v_threshold, v_reset=v_reset, surrogate_function=surrogate.ATan(), detach_reset=True),
#         )
#         self.conv2 = nn.Sequential(
#             # nn.Conv2d(in_channels=64, out_channels=128, kernel_size=5, stride=2, padding=2, bias=False),
#             nn.Conv2d(in_channels=C[1], out_channels=C[1], groups=1, kernel_size=K, stride=2, padding=P, bias=False),
#             nn.Conv2d(in_channels=C[1], out_channels=C[2], kernel_size=1, stride=1, bias=False),
#             MultiplyBy(multiply_factor),
#             neuron.ParametricLIFNode(init_tau=tau, v_threshold=v_threshold, v_reset=v_reset, detach_reset=True, surrogate_function=surrogate_function) if use_plif else neuron.LIFNode(tau=tau, v_threshold=v_threshold, v_reset=v_reset, surrogate_function=surrogate.ATan(), detach_reset=True),
#         )
#         self.conv3 = nn.Sequential(
#             # nn.Conv2d(in_channels=128, out_channels=256, kernel_size=5, stride=2, padding=2, bias=False),
#             nn.Conv2d(in_channels=C[2], out_channels=C[2], groups=1, kernel_size=K, stride=2, padding=P, bias=False),
#             nn.Conv2d(in_channels=C[2], out_channels=C[3], kernel_size=1, stride=1, bias=False),
#             MultiplyBy(multiply_factor),
#             neuron.ParametricLIFNode(init_tau=tau, v_threshold=v_threshold, v_reset=v_reset, detach_reset=True, surrogate_function=surrogate_function) if use_plif else neuron.LIFNode(tau=tau, v_threshold=v_threshold, v_reset=v_reset, surrogate_function=surrogate.ATan(), detach_reset=True),
#         )
#         self.conv4 = nn.Sequential(
#             # nn.Conv2d(in_channels=256, out_channels=512, kernel_size=5, stride=2, padding=2, bias=False),
#             nn.Conv2d(in_channels=C[3], out_channels=C[3], groups=1, kernel_size=K, stride=2, padding=P, bias=False),
#             nn.Conv2d(in_channels=C[3], out_channels=C[4], kernel_size=1, stride=1, bias=False),
#             MultiplyBy(multiply_factor),
#             neuron.ParametricLIFNode(init_tau=tau, v_threshold=v_threshold, v_reset=v_reset, detach_reset=True, surrogate_function=surrogate_function) if use_plif else neuron.LIFNode(tau=tau, v_threshold=v_threshold, v_reset=v_reset, surrogate_function=surrogate.ATan(), detach_reset=True),
#         )
...
#         # residual layers
#         self.bottleneck = nn.Sequential(
#             SeparableSEWResBlock_Group1(C[4], kernel_size=K, v_threshold=v_threshold, v_reset=v_reset, connect_function='ADD', multiply_factor=multiply_factor, use_plif=True, tau=tau, surrogate_function=surrogate_function),
#             SeparableSEWResBlock_Group1(C[4], kernel_size=K, v_threshold=v_threshold, v_reset=v_reset, connect_function='ADD', multiply_factor=multiply_factor, use_plif=True, tau=tau, surrogate_function=surrogate_function),
#         )
...
# class PostTrainingQuantizedModel_Group1(fromZero_feedforward_multiscale_tempo_Matt_sepConv_SpikeFlowNetLike_Group1):
#     """
#     Post-Training Quantization of fromZero_feedforward_multiscale_tempo_Matt_sepConv_SpikeFlowNetLike.
#     """
#     def __init__(self, *args, **kwargs):
...
#     @staticmethod
#     def reformat_input_data_quant(warmup_chunks_left, warmup_chunks_right, inference_chunks_left, inference_chunks_right):
#         # Ensure all inputs are dequantized before performing operations
#         if warmup_chunks_left.is_quantized:
#             warmup_chunks_left = warmup_chunks_left.dequantize()
#         if warmup_chunks_right.is_quantized:
#             warmup_chunks_right = warmup_chunks_right.dequantize()
#         if inference_chunks_left.is_quantized:
#             inference_chunks_left = inference_chunks_left.dequantize()
#         if inference_chunks_right.is_quantized:
#             inference_chunks_right = inference_chunks_right.dequantize()
...
#         depth_outputs, activations = super().forward(x)
#         print("depth_outputs")
#         depth_outputs = self.dequant(depth_outputs)
#         print("dequant")
#         return depth_outputs, activations