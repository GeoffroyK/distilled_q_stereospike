...
def mvsecRectifyEvents_inplace(events, x_map, y_map):
    """
    DOES THE SAME THING, BUT INPLACE (FOR LESS MEMORY CONSUMPTION)
    """
    print("\nrectifying spike coordinates...")
    for i in tqdm(range(len(events))):
        ...
        x_rect = x_map[y, x]
        y_rect = y_map[y, x]
        events[i] = [x_rect, y_rect, events[i][2], events[i][3]]
...
def mvsecCumulateSpikesIntoFrames_offline(side, events, depth_rect, depth_rect_ts, out_path, num_frames_per_depth_map=1):
    """
    DOES THE SAME THING, BUT DATA PREPROCESSING AND LOADING IS DONE OFFLINE

    side argument must be a string "L" or "R"
    """
    assert num_frames_per_depth_map in [1, 2, 5, 10, 25], "num_frames_per_depth_map must divide 50 ! Choose another " \
                                                          "value among [1, 2, 5, 10, 25] ..."
    print("\nCumulating spikes into frames and synchronizing with ground-truth...")
    ...
    # create folders to hold input data and labels
    if '{}chunks'.format(side) not in os.listdir(out_path):
        os.mkdir(out_path + '/{}chunks'.format(side))
    if 'labels' not in os.listdir(out_path):
    ...
    for numchunk in tqdm(range(len(depth_rect_ts))):  # we have as many chunks as groundtruth maps
        ...
        torch.save(np.array(chunk), out_path+'/{}chunks/'.format(side)+'{}chunk{}_nfpdm{}.pt'.format(side, numchunk, num_frames_per_depth_map))
        torch.save(np.array(depth_rect[numchunk]), out_path+'/labels/'+'label{}.pt'.format(numchunk))


def mvsecCumulateSpikesIntoFrames_offline_opt(side, events, depth_rect, depth_rect_ts, out_path, num_frames_per_depth_map=1):
    """
    DOES THE SAME THING, BUT DATA PREPROCESSING AND LOADING IS DONE OFFLINE
    ...
    # create folders to hold input data and labels
    if out_path + '/{}chunks'.format(side) not in os.listdir(out_path):
        os.mkdir(out_path + '/{}chunks'.format(side))
    if out_path + '/labels' not in os.listdir(out_path):
        os.mkdir(out_path + '/labels')
    ...
    for numchunk in tqdm(range(len(depth_rect_ts))):  # we have as many chunks as groundtruth maps
        ...
        for numframe in range(num_frames_per_depth_map):  # we get num_frames_per_depth_map frames per chunk
            ...
            # events that occur between the timestamps of the current and the next frame
            end_ts = numchunk*num_frames_per_depth_map*1/fps + numframe*1/fps + 1/fps
            filt_indices = np.where(events[start_index:, 2] < end_ts)
            filt_events = events[filt_indices]
            ...
            start_index = filt_indices[-1] + 1

        torch.save(np.array(chunk), out_path+'/{}chunks/{}chunk{}_nfpdm{}.pt'.format(side, side, numchunk, num_frames_per_depth_map))
        torch.save(np.array(depth_rect[numchunk]), out_path+'/labels/'+'label{}.pt'.format(numchunk))


def mvsecLoadPreprocessedDataFromFolders(root):
    """
    Load data preprocessed offline and return L/R input data + groundtruth

    :param root: path of the root folder that must contain 3 folders "Lchunks", "Rchunks" and "labels"
    :return: an np.array for each of Lchunks, Rchunks and labels
    """
    import re
    ...
    def loadFolder(path):
        """load a folder containing tensors saved with torch.save and return them as a single array"""
        sorted_filenames = os.listdir(path)
        sorted_filenames.sort(key=natural_keys)

        lst = []
        for elem_name in tqdm(sorted_filenames):
            elem_path = os.path.join(path, elem_name)
            elem = torch.load(elem_path)
            lst.append(elem)
        return np.array(lst)

    Lchunks_path = os.path.join(root, "Lchunks")
    Rchunks_path = os.path.join(root, "Rchunks")
    labels_path = os.path.join(root, "labels")

    #print("hello")
    #sorted_filenames = os.listdir(labels_path)
    #sorted_filenames.sort(key=natural_keys)
    #print(sorted_filenames)

    xL = loadFolder(Lchunks_path)
    xR = loadFolder(Rchunks_path)
    yL = loadFolder(labels_path)
