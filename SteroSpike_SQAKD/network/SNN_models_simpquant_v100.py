...
from .custom_modules_v100 import QConv, QConv_PW, QConv_DW


class SeparableSEWResBlock_Noskip(nn.Module):
    """
    ...
    def __init__(self, in_channels: int, connect_function='ADD', v_threshold=1., v_reset=0.,
        ...
        super(SeparableSEWResBlock_Noskip, self).__init__()
...
class SQAKD_QANTIZED_SeparableSEWResBlock_Noskip(nn.Module):
    """
    Version of the SEW-Resblock using Separable Convolutions
    """
    ...
    def __init__(self, in_channels: int, connect_function='ADD', v_threshold=1., v_reset=0.,
                 surrogate_function=surrogate.Sigmoid(), use_plif=False, tau=2., multiply_factor=1., kernel_size=3):
        super(SQAKD_QANTIZED_SeparableSEWResBlock_Noskip, self).__init__()

        self.conv1 = nn.Sequential(
            # nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=1, padding=(3 - 1) // 2, bias=False),
            QConv(in_channels, in_channels, groups=in_channels, kernel_size=kernel_size, stride=1, padding=(kernel_size - 1) // 2, bias=False),
            QConv(in_channels, in_channels, kernel_size=1, stride=1, bias=False),
            MultiplyBy(multiply_factor),
            # nn.BatchNorm2d(in_channels),
        )
...
class SQAKD_v2_QANTIZED_SeparableSEWResBlock_Noskip(nn.Module):
    """
    Version of the SEW-Resblock using Separable Convolutions
    """
    ...
    def __init__(self, in_channels: int, connect_function='ADD', v_threshold=1., v_reset=0.,
                 surrogate_function=surrogate.Sigmoid(), use_plif=False, tau=2., multiply_factor=1., kernel_size=3):
        super(SQAKD_v2_QANTIZED_SeparableSEWResBlock_Noskip, self).__init__()

        self.conv1 = nn.Sequential(
            # nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=1, padding=(3 - 1) // 2, bias=False),
            QConv_DW(in_channels, in_channels, groups=in_channels, kernel_size=kernel_size, stride=1, padding=(kernel_size - 1) // 2, bias=False),
            QConv_PW(in_channels, in_channels, kernel_size=1, stride=1, bias=False),
            MultiplyBy(multiply_factor),
            # nn.BatchNorm2d(in_channels),
        )
...
class QUANTIZABLE_SIMPLIFIED_SeparableSEWResBlock_noskip(nn.Module):
    """
    Quantization-compatible version of the simplified resblock
    """
    ...
    def __init__(self, in_channels: int, connect_function='ADD', v_threshold=1., v_reset=0.,
                 surrogate_function=surrogate.Sigmoid(), use_plif=False, tau=2., multiply_factor=1., kernel_size=3):
        super(QUANTIZABLE_SIMPLIFIED_SeparableSEWResBlock_noskip, self).__init__()
...
class SQAKD_QUANTIZABLE_SIMPLIFIED_SeparableSEWResBlock_noskip(nn.Module):
    """
    Quantization-compatible version of the simplified resblock
    """
    ...
    def __init__(self, in_channels: int, connect_function='ADD', v_threshold=1., v_reset=0.,
                 surrogate_function=surrogate.Sigmoid(), use_plif=False, tau=2., multiply_factor=1., kernel_size=3):
        super(SQAKD_QUANTIZABLE_SIMPLIFIED_SeparableSEWResBlock_noskip, self).__init__()
        ...
        self.conv1 = nn.Sequential(
            # nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=1, padding=(3 - 1) // 2, bias=False),
            QConv(in_channels=in_channels, out_channels=in_channels, groups=in_channels, kernel_size=kernel_size, stride=1, padding=(kernel_size - 1) // 2, bias=False),
            QConv(in_channels=in_channels, out_channels=in_channels, kernel_size=1, stride=1, bias=False),
        )
...
class SIMPLIFIED_SeparableSEWResBlock_noskip_intermediatespike(nn.Module):
    """
    Version of the SEW-Resblock using Separable Convolutions
    """
    ...
    def __init__(self, in_channels: int, connect_function='ADD', v_threshold=1., v_reset=0.,
                 surrogate_function=surrogate.Sigmoid(), use_plif=False, tau=2., multiply_factor=1., kernel_size=3):
        super(SIMPLIFIED_SeparableSEWResBlock_noskip_intermediatespike, self).__init__()

        self.conv1 = nn.Sequential(
            # nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=1, padding=(3 - 1) // 2, bias=False),
            nn.Conv2d(in_channels, in_channels, groups=in_channels, kernel_size=kernel_size, stride=1, padding=(kernel_size - 1) // 2, bias=False),
            surrogate.ATan(spiking=True),
            nn.Conv2d(in_channels, in_channels, kernel_size=1, stride=1, bias=False),
        )
...
class fromZero_feedforward_multiscale_tempo_Matt_Noskip_sepConv_SpikeFlowNetLike(NeuromorphicNet):
    """
    Uses separable convolutions for a lighter model
...
class fromZero_feedforward_multiscale_tempo_Matt_NoskipAll_sepConv_SpikeFlowNetLike_v2(NeuromorphicNet):
    """
    Uses separable convolutions for a lighter model
    ...
    def __init__(self, input_chans=4, kernel_size=7, base_chans=32, use_plif=False, detach_reset=True, tau=10., v_threshold=1.0, v_reset=0.0, multiply_factor=1., surrogate_function=surrogate.Sigmoid(), learnable_biases=False):
        ...
        # residual layers
        self.bottleneck = nn.Sequential(
            SeparableSEWResBlock_Noskip(C[4], kernel_size=K, v_threshold=v_threshold, v_reset=v_reset, connect_function='ADD', multiply_factor=multiply_factor, use_plif=True, tau=tau, surrogate_function=surrogate_function),
            SeparableSEWResBlock_Noskip(C[4], kernel_size=K, v_threshold=v_threshold, v_reset=v_reset, connect_function='ADD', multiply_factor=multiply_factor, use_plif=True, tau=tau, surrogate_function=surrogate_function),
        )
...
class SIMPLIFIED_fromZero_feedforward_multiscale_tempo_Matt_Noskip_sepConv_SpikeFlowNetLike(NeuromorphicNet):
    """
    Simplified but mathematically equivalent version of 'fromZero_feedforward_multiscale_tempo_Matt_sepConv_SpikeFlowNetLike'
...
class SIMPLIFIED_fromZero_feedforward_multiscale_tempo_Matt_NoskipAll_sepConv_SpikeFlowNetLike_v2(NeuromorphicNet):
    """
    Simplified but mathematically equivalent version of 'fromZero_feedforward_multiscale_tempo_Matt_sepConv_SpikeFlowNetLike'
...
class SIMPLIFIED_fromZero_feedforward_multiscale_tempo_Matt_NoskipAll_sepConv_intermediatespike_SpikeFlowNetLike_v2(NeuromorphicNet):
    """
    Simplified but mathematically equivalent version of 'fromZero_feedforward_multiscale_tempo_Matt_sepConv_SpikeFlowNetLike'
    ...
    def __init__(self, input_chans=4, kernel_size=7, base_chans=32, use_plif=False, detach_reset=True, tau=10., v_threshold=1.0, v_reset=0.0, multiply_factor=1., surrogate_function=surrogate.Sigmoid(), learnable_biases=False):
        ...
        # residual layers
        self.bottleneck = nn.Sequential(
            SIMPLIFIED_SeparableSEWResBlock_noskip_intermediatespike(C[4], kernel_size=K, v_threshold=v_threshold, v_reset=v_reset, multiply_factor=multiply_factor, use_plif=True, tau=tau, surrogate_function=surrogate_function),
            SIMPLIFIED_SeparableSEWResBlock_noskip_intermediatespike(C[4], kernel_size=K, v_threshold=v_threshold, v_reset=v_reset, multiply_factor=multiply_factor, use_plif=True, tau=tau, surrogate_function=surrogate_function),
        )

        # decoder layers (upsampling)
        self.deconv4 = nn.Sequential(
            SeparableNNConvUpsampling_intermediatespike(in_channels=C[4], out_channels=C[3], kernel_size=K, up_size=(33, 44)),
            #neuron.IFNode()
        )
        self.deconv3 = nn.Sequential(
            SeparableNNConvUpsampling_intermediatespike(in_channels=C[3], out_channels=C[2], kernel_size=K, up_size=(65, 87)),
            #neuron.IFNode()
        )
        self.deconv2 = nn.Sequential(
            SeparableNNConvUpsampling_intermediatespike(in_channels=C[2], out_channels=C[1], kernel_size=K, up_size=(130, 173)),
            #neuron.IFNode()
        )
        self.deconv1 = nn.Sequential(
            SeparableNNConvUpsampling_intermediatespike(in_channels=C[1], out_channels=C[0], kernel_size=K, up_size=(260, 346)),
            #neuron.IFNode()
        )

        # these layers output depth maps at different scales, where depth is represented by the potential of IF neurons
        # that do not fire ("I-neurons"), i.e., with an infinite threshold.
        self.predict_depth4 = SeparableNNConvUpsampling_intermediatespike(in_channels=C[3], out_channels=1, kernel_size=K, up_size=(260, 346), bias=True)
        self.predict_depth3 = SeparableNNConvUpsampling_intermediatespike(in_channels=C[2], out_channels=1, kernel_size=K, up_size=(260, 346), bias=True)
        self.predict_depth2 = SeparableNNConvUpsampling_intermediatespike(in_channels=C[1], out_channels=1, kernel_size=K, up_size=(260, 346), bias=True)
        self.predict_depth1 = SeparableNNConvUpsampling_intermediatespike(in_channels=C[0], out_channels=1, kernel_size=K, up_size=(260, 346), bias=True)
...
class QUANTIZABLE_SIMPLIFIED_fromZero_feedforward_multiscale_tempo_Matt_Noskip_sepConv_SpikeFlowNetLike(NeuromorphicNet):
    """
    Quantization-compatible version of the simplified model
    ...
    def forward(self, x):
        ...
        out_deconv4 = self.quant(torch.heaviside(self.dequant(self.deconv4(out_rconv)) - 1., torch.tensor(0.0, device=x.device)))
        #out_add4 = self.skip_add.add(out_deconv4, out_conv3)
        depth4 = self.predict_depth4(out_deconv4) 

        out_deconv3 = self.quant(torch.heaviside(self.dequant(self.deconv3(out_deconv4)) - 1., torch.tensor(0.0, device=x.device)))
        #out_add3 = self.skip_add.add(out_deconv3, out_conv2)
        depth3 = self.pred_add.add(depth4, self.predict_depth3(out_deconv3)) 

        out_deconv2 = self.quant(torch.heaviside(self.dequant(self.deconv2(out_deconv3)) - 1., torch.tensor(0.0, device=x.device)))
        #out_add2 = self.skip_add.add(out_deconv2, out_conv1)
        depth2  = self.pred_add.add(depth3, self.predict_depth2(out_deconv2))

        out_deconv1 = self.quant(torch.heaviside(self.dequant(self.deconv1(out_deconv2)) - 1., torch.tensor(0.0, device=x.device)))
        #out_add1 = self.skip_add.add(out_deconv1, out_bottom)
        depth1 = self.pred_add.add(depth2, self.predict_depth1(out_deconv1))
        ...
        depth1 = self.dequant(depth1)
        out_rconv = self.dequant(out_rconv)
        out_add4 = self.dequant(out_deconv4)
        out_add3 = self.dequant(out_deconv3)
        out_add2 = self.dequant(out_deconv2)
        out_add1 = self.dequant(out_deconv1)

        # the membrane potentials of the output IF neuron carry the depth prediction
        return [depth1, depth2, depth3, depth4], [out_rconv, out_add4, out_add3, out_add2, out_add1]
    

class QUANTIZABLE_SIMPLIFIED_fromZero_feedforward_multiscale_tempo_Matt_NoskipAll_sepConv_SpikeFlowNetLike_v2(NeuromorphicNet):
    """
    Quantization-compatible version of the simplified model
    ...
    def __init__(self, input_chans=4, kernel_size=7, base_chans=32, use_plif=False, detach_reset=True, tau=10., v_threshold=1.0, v_reset=0.0, multiply_factor=1., surrogate_function=surrogate.Sigmoid(), learnable_biases=False):
        ...
        # residual layers
        self.bottleneck = nn.Sequential(
            QUANTIZABLE_SIMPLIFIED_SeparableSEWResBlock_noskip(C[4], kernel_size=K, v_threshold=v_threshold, v_reset=v_reset, multiply_factor=multiply_factor, use_plif=True, tau=tau, surrogate_function=surrogate_function),
            QUANTIZABLE_SIMPLIFIED_SeparableSEWResBlock_noskip(C[4], kernel_size=K, v_threshold=v_threshold, v_reset=v_reset, multiply_factor=multiply_factor, use_plif=True, tau=tau, surrogate_function=surrogate_function),
        )
...
class SQAKD_v2_QUANTIZABLE_SIMPLIFIED_fromZero_feedforward_multiscale_tempo_Matt_NoskipAll_sepConv_SpikeFlowNetLike_v2(NeuromorphicNet):
    """
    Quantization-compatible version of the simplified model for SQAKD
    ...
    def __init__(self, input_chans=4, kernel_size=7, base_chans=32, use_plif=False, detach_reset=True, tau=10., v_threshold=1.0, v_reset=0.0, multiply_factor=1., surrogate_function=surrogate.Sigmoid(), learnable_biases=False):
        ...
        self.bottom = nn.Sequential(
            ...
            QConv(in_channels=input_chans, out_channels=C[0], kernel_size=1, stride=1, bias=False),
            #neuron.IFNode()
        )
        ...
        self.conv1 = nn.Sequential(
            # nn.Conv2d(in_channels=32, out_channels=64, kernel_size=5, stride=2, padding=2, bias=False),
            QConv(in_channels=C[0], out_channels=C[0], groups=C[0], kernel_size=K, stride=2, padding=P, bias=False),
            QConv(in_channels=C[0], out_channels=C[1], kernel_size=1, stride=1, bias=False),
            #neuron.IFNode()
        )
        self.conv2 = nn.Sequential(
            # nn.Conv2d(in_channels=64, out_channels=128, kernel_size=5, stride=2, padding=2, bias=False),
            QConv(in_channels=C[1], out_channels=C[1], groups=C[1], kernel_size=K, stride=2, padding=P, bias=False),
            QConv(in_channels=C[1], out_channels=C[2], kernel_size=1, stride=1, bias=False),
            #neuron.IFNode(),
        )
        self.conv3 = nn.Sequential(
            # nn.Conv2d(in_channels=128, out_channels=256, kernel_size=5, stride=2, padding=2, bias=False),
            QConv(in_channels=C[2], out_channels=C[2], groups=C[2], kernel_size=K, stride=2, padding=P, bias=False),
            QConv(in_channels=C[2], out_channels=C[3], kernel_size=1, stride=1, bias=False),
            #neuron.IFNode()
        )
        self.conv4 = nn.Sequential(
            # nn.Conv2d(in_channels=256, out_channels=512, kernel_size=5, stride=2, padding=2, bias=False),
            QConv(in_channels=C[3], out_channels=C[3], groups=C[3], kernel_size=K, stride=2, padding=P, bias=False),
            QConv(in_channels=C[3], out_channels=C[4], kernel_size=1, stride=1, bias=False),
            #neuron.IFNode(),
        )

        # residual layers
        self.bottleneck = nn.Sequential(
            SQAKD_QUANTIZABLE_SIMPLIFIED_SeparableSEWResBlock_noskip(C[4], kernel_size=K, v_threshold=v_threshold, v_reset=v_reset, multiply_factor=multiply_factor, use_plif=True, tau=tau, surrogate_function=surrogate_function),
            SQAKD_QUANTIZABLE_SIMPLIFIED_SeparableSEWResBlock_noskip(C[4], kernel_size=K, v_threshold=v_threshold, v_reset=v_reset, multiply_factor=multiply_factor, use_plif=True, tau=tau, surrogate_function=surrogate_function),
        )

        # decoder layers (upsampling)
        self.deconv4 = nn.Sequential(
            SQAKD_SeparableNNConvUpsampling(in_channels=C[4], out_channels=C[3], kernel_size=K, up_size=(33, 44)),
            #neuron.IFNode()
        )
        self.deconv3 = nn.Sequential(
            SQAKD_SeparableNNConvUpsampling(in_channels=C[3], out_channels=C[2], kernel_size=K, up_size=(65, 87)),
            #neuron.IFNode()
        )
        self.deconv2 = nn.Sequential(
            SQAKD_SeparableNNConvUpsampling(in_channels=C[2], out_channels=C[1], kernel_size=K, up_size=(130, 173)),
            #neuron.IFNode()
        )
        self.deconv1 = nn.Sequential(
            SQAKD_SeparableNNConvUpsampling(in_channels=C[1], out_channels=C[0], kernel_size=K, up_size=(260, 346)),
            #neuron.IFNode()
        )

        # these layers output depth maps at different scales, where depth is represented by the potential of IF neurons
        # that do not fire ("I-neurons"), i.e., with an infinite threshold.
        self.predict_depth4 = SQAKD_SeparableNNConvUpsampling(in_channels=C[3], out_channels=1, kernel_size=K, up_size=(260, 346), bias=True)
        self.predict_depth3 = SQAKD_SeparableNNConvUpsampling(in_channels=C[2], out_channels=1, kernel_size=K, up_size=(260, 346), bias=True)
        self.predict_depth2 = SQAKD_SeparableNNConvUpsampling(in_channels=C[1], out_channels=1, kernel_size=K, up_size=(260, 346), bias=True)
        self.predict_depth1 = SQAKD_SeparableNNConvUpsampling(in_channels=C[0], out_channels=1, kernel_size=K, up_size=(260, 346), bias=True)
...
class SQAKD_fromZero_feedforward_multiscale_tempo_Matt_NoskipAll_sepConv_SpikeFlowNetLike_v3(NeuromorphicNet):
    """
    Uses separable convolutions for a lighter model
...
class SQAKD_QUANTIZABLE_fromZero_feedforward_multiscale_tempo_Matt_NoskipAll_sepConv_SpikeFlowNetLike_v3(NeuromorphicNet):
    """
    Uses separable convolutions for a lighter model
    ...
    def __init__(self, input_chans=4, kernel_size=7, base_chans=32, use_plif=False, detach_reset=True, tau=10., v_threshold=1.0, v_reset=0.0, multiply_factor=1., surrogate_function=surrogate.Sigmoid(), learnable_biases=False):
        ...
        self.bottom = nn.Sequential(
            # nn.Conv2d(in_channels=4, out_channels=32, kernel_size=5, stride=1, padding=2, bias=False),
            QConv(in_channels=input_chans, out_channels=input_chans, groups=input_chans, kernel_size=K, stride=1, padding=P, bias=False),
            QConv(in_channels=input_chans, out_channels=C[0], kernel_size=1, stride=1, bias=False),
            MultiplyBy(multiply_factor),
            neuron.ParametricLIFNode(init_tau=tau, v_threshold=v_threshold, v_reset=v_reset, detach_reset=True, surrogate_function=surrogate_function) if use_plif else neuron.LIFNode(tau=tau, v_threshold=v_threshold, v_reset=v_reset, surrogate_function=surrogate.ATan(), detach_reset=True),
        )
        ...
        # residual layers
        self.bottleneck = nn.Sequential(
            SQAKD_QANTIZED_SeparableSEWResBlock_Noskip(C[4], kernel_size=K, v_threshold=v_threshold, v_reset=v_reset, connect_function='ADD', multiply_factor=multiply_factor, use_plif=True, tau=tau, surrogate_function=surrogate_function),
            SQAKD_QANTIZED_SeparableSEWResBlock_Noskip(C[4], kernel_size=K, v_threshold=v_threshold, v_reset=v_reset, connect_function='ADD', multiply_factor=multiply_factor, use_plif=True, tau=tau, surrogate_function=surrogate_function),
        )
        ...
        # that do not fire ("I-neurons"), i.e., with an infinite threshold.
        self.predict_depth4 = nn.Sequential(
            SQAKD_SeparableNNConvUpsampling(in_channels=C[3], out_channels=1, kernel_size=K, up_size=(260, 346), bias=True),
            MultiplyBy(multiply_factor),
        )
        self.predict_depth3 = nn.Sequential(
            SQAKD_SeparableNNConvUpsampling(in_channels=C[2], out_channels=1, kernel_size=K, up_size=(260, 346), bias=True),
            MultiplyBy(multiply_factor),
        )
        self.predict_depth2 = nn.Sequential(
            SQAKD_SeparableNNConvUpsampling(in_channels=C[1], out_channels=1, kernel_size=K, up_size=(260, 346), bias=True),
            MultiplyBy(multiply_factor),
        )
        self.predict_depth1 = nn.Sequential(
            SQAKD_SeparableNNConvUpsampling(in_channels=C[0], out_channels=1, kernel_size=K, up_size=(260, 346), bias=True),
            MultiplyBy(multiply_factor),
        )
