...
from network.models.custom_modules import QConv
...
def init_quant_model(model, train_loader, device, distill=None):
    ...
    if distill == 'crd' or distill == 'crdst':
        ...
        init_pots, warmup_chunks_left, warmup_chunks_right, train_chunks_left, train_chunks_right, label = next(iterloader)
    ...
    model.train()
    warmup_chunks, train_chunks = model.reformat_input_data(warmup_chunks_left, warmup_chunks_right, train_chunks_left, train_chunks_right)
    binary_chunks = (train_chunks >= 1).to(train_chunks.dtype)
    #model.forward(images)
    model.forward(binary_chunks)
    for m in model.modules():
        if isinstance(m, QConv):
            m.init.data.fill_(0)
