...
# net = fromZero_feedforward_multiscale_tempo_Matt_sepConv_SpikeFlowNetLike(input_chans=2*2*1, tau=3., v_threshold=1.0, v_reset=0.0, use_plif=True, multiply_factor=10., surrogate_function=surrogate.ATan(), learnable_biases=False)
...
class fromZero_feedforward_multiscale_tempo_Matt_sepConv_SpikeFlowNetLike(NeuromorphicNet):
	...
	self.bottom[0].weight = self.bottom[0].weight * multiply_factor
	self.bottom[0].bias = self.bottom[0].bias * multiply_factor
	...
	self.conv1[0].weight = self.conv1[0].weight * multiply_factor
	self.conv1[0].bias = self.conv1[0].bias * multiply_factor
	...
	self.conv2[0].weight = self.convé[0].weight * multiply_factor
	self.conv2[0].bias = self.conv2[0].bias * multiply_factor
	...
	self.conv3[0].weight = self.conv3[0].weight * multiply_factor
	self.conv3[0].bias = self.conv3[0].bias * multiply_factor
	...
	self.conv4[0].weight = self.conv4[0].weight * multiply_factor
	self.conv4[0].bias = self.conv4[0].bias * multiply_factor
...
class quantized_OPT_fromZero_feedforward_multiscale_tempo_Matt_sepConv_SpikeFlowNetLike(NeuromorphicNet):
    ...
    def forward(self, x):
        ...
        # data is fed in through the bottom layer
        out_bottom = self.bottom(frame)
        #out_bottom_S = torch.zeros(out_bottom.shape)  # uncommment !
        #torch.gt(out_bottom, torch.zeros(out_bottom.shape), out_bottom_S)  # uncomment ! 
