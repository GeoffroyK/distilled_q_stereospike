...
from .utils import mvsecLoadRectificationMaps, mvsecRectifyEvents, mvsecCumulateSpikesIntoFrames, \
        mvsecRectifyEvents_inplace, mvsecCumulateSpikesIntoFrames_offline, mvsecLoadPreprocessedDataFromFolders
from .indices import *
...
def load_MVSEC(root: str, scenario:str, split:str, num_frames_per_depth_map, warmup_chunks, train_chunks,
    ...
    """
        Load an indoor split of MVSEC in only one function.
...
class MVSEC_sequence_offline(Dataset):
    """
    ...
    def __init__(self, root: str, scenario: str, split: str, sequence: str,
        ...
        scenario_path = os.path.join(root, scenario) # e.g., ".datasets/MVSEC/data/outdoor_day/"
        sequence_path = os.path.join(root, scenario, scenario+sequence) # e.g., ".datasets/MVSEC/data/outdoor_day/outdoor_day2/

        if scenario+sequence not in os.listdir(scenario_path):
            ...
            print("preprocessing hdf5 file...")
            print("writing to chunks/ and labels/")
            ...
            Ry_path = self.root + '{}/{}_calib/{}_right_y_map.txt'.format(scenario, scenario, scenario)
            Lx_map, Ly_map, Rx_map, Ry_map = mvsecLoadRectificationMaps(Lx_path, Ly_path, Rx_path, Ry_path)
            Levents = mvsecRectifyEvents_inplace(Levents, Lx_map, Ly_map)
            Revents = mvsecRectifyEvents_inplace(Revents, Rx_map, Ry_map)

            # make histograms from spikes and save them in a folder
            Levents = mvsecCumulateSpikesIntoFrames_offline("L", Levents, Ldepths_rect, Ldepths_rect_ts, out_path=sequence_path, num_frames_per_depth_map=num_frames_per_depth_map)
            Revents = mvsecCumulateSpikesIntoFrames_offline("R", Revents, Ldepths_rect, Ldepths_rect_ts, out_path=sequence_path, num_frames_per_depth_map=num_frames_per_depth_map)
            ...
            print("found existing folders of preprocessed data: skipping preprocessing...\n")
        ...
        xL, xR, yL = mvsecLoadPreprocessedDataFromFolders(sequence_path)
        assert xL.shape == xR.shape

        # convert linear (metric) depth to log depth or disparity if required
        if learn_on == 'LOG':
            yL = lin_to_log_depths(yL)
        elif learn_on == 'DISP':
            yL = depth_to_disparity(yL)
        elif learn_on == 'LIN':
            pass
        else:
            raise ValueError("'learn_on' argument should either be 'LIN' for metric depth, "
